    ...
    <div className="space-y-2">
      ...
      {viewMode === 'list' && (
        <div className="space-y-1">
          {showDetailedMobile ? (
            <>
              <div className="grid grid-cols-[minmax(0,1fr)_minmax(48px,auto)_minmax(48px,auto)_minmax(52px,auto)] items-center gap-x-1 text-[9px] text-ancient-stone px-1.5 pb-1 opacity-70">
                <span>资源</span>
              ...
              {visibleResources.map(([key, info]) => {
                ...
                return (
                  <button
                    key={key}
                    onClick={() => onDetailClick && onDetailClick(key)}
                    className="w-full relative group grid grid-cols-[minmax(0,1fr)_minmax(48px,auto)_minmax(48px,auto)_minmax(52px,auto)] items-center gap-x-1 text-[10px] sm:text-xs p-1.5 rounded-lg transition-all cursor-pointer overflow-hidden border border-ancient-gold/10 hover:border-ancient-gold/30 hover:bg-ancient-gold/5 hover:shadow-glow-gold touch-feedback"
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-ancient-gold/0 via-ancient-gold/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

                    <div className="flex items-center gap-1 overflow-hidden relative z-10 min-w-0">
                    ...
                    <span className="font-mono font-bold text-ancient-parchment text-right relative z-10 group-hover:text-ancient text-[12px]">
                      {formatCompactNumber(amount)}
                    </span>

                    <div className="flex items-center justify-end gap-0.5 font-mono text-ancient-stone font-bold text-[11px] sm:text-[13px] relative z-10 group-hover:text-ancient-gold">
                      <span>{price.toFixed(1)}</span>
                      <Icon name="Coins" size={8} className="text-ancient-gold/70 flex-shrink-0" />
                    </div>

                    <span className={`font-mono text-right relative z-10 transition-colors text-[9px] ${rateColorClass}`}>
                      {rateDisplay}
                    </span>
                  </button>
                );
