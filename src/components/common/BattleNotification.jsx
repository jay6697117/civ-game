...
// 配置常量
const MAX_VISIBLE_NOTIFICATIONS = 1;  // 一次只显示1条通知（队列模式）
...
const NotificationItem = ({ notification, onView, onDismiss, autoDismissDelay = AUTO_DISMISS_DELAY }) => {
    ...
    return (
        <div
            className={`relative flex items-center gap-2 px-3 py-2 rounded-lg border shadow-lg backdrop-blur-sm cursor-pointer transition-all duration-200 overflow-hidden ${result.victory
        ...
        >
            {/* 进度条 - 显示剩余时间 */}
            <div
                className={`absolute bottom-0 left-0 h-0.5 transition-all ${result.victory ? 'bg-emerald-400/60' : 'bg-red-400/60'
                    }`}
                style={{ width: `${progress}%` }}
            />

            {/* 图标 */}
            <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${result.victory ? 'bg-emerald-700/50' : 'bg-red-700/50'
                }`}>
                <Icon
...
export const BattleNotification = ({ notifications = [], onViewDetail, onDismiss, onDismissAll }) => {
    ...
    // 队列模式：只显示第一个（最早的）通知，后续通知等待
    const visibleNotifications = notifications.slice(0, MAX_VISIBLE_NOTIFICATIONS);
    const queuedCount = notifications.length - visibleNotifications.length;
